...
export class CursoService extends UnsubscribeOnDestroyAdapter {
  ...
  getAllCursos(idEstudiante){
    const params = new HttpParams()
      ...
      if(idEstudiante==0){
        ...
        this.subs.sink = this.httpClient.get<CursosResponse>(this.API_URL+ "/cursos",{params}).subscribe(
          (data) => {
            this.isTblLoading = false;
            this.dataChange.next(data.data);
          },
  ...
  addCurso(curso: Curso){
    var json={
      idCurso:curso.id.toString()
    };
    return this.httpClient.post(this.API_URL+ "/registro_cursos", json);
  }
